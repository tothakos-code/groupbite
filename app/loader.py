...
def load_plugins(db, plugin_dirs: list) -> None:
    for plugin_dir in plugin_dirs:
        ...
        try:
            ...
            logging.exception(f"{plugin_dir} plugin can not be imported, module not found: {e}")
            continue
        ...
        if vendor:
            logging.info(f"Plugin vendor '{vendor_name}' already exists (id: {vendor.id})")
        else:
            vendor = Vendor(
                name=vendor_name,
                type=VendorType.PLUGIN,
        ...
        logging.info(f"Registered service {service_class.__name__} for vendor {vendor.id}")
...
def _find_service_class(module) -> Type[BaseVendorService]:
    for name, obj in module.__dict__.items():
        if (isinstance(obj, type) and
                issubclass(obj, BaseVendorService) and
                obj != BaseVendorService):
            return obj
    return None