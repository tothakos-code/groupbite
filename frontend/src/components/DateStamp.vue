<template>
  <div class="row d-flex justify-content-center align-items-center">
    <div class="col text-end">
      <button type="button" name="button" class="btn btn-link link-warning" :disabled="this.limitToCurrentWeek && this.currentDate.getDay() == 1" @click="this.prevDay()">
    ...
    <div class="col text-start">
      <button type="button" name="button" class="btn btn-link link-warning" :disabled="this.limitToCurrentWeek && this.currentDate.getDay() == 0" @click="this.nextDay()">
...
export default {
  ...
  methods: {
    nextDay: function() {
      if (this.limitToCurrentWeek && this.currentDate.getDay() == 0) {
        return;
      }
      const newDate = new Date(this.currentDate);
      newDate.setDate(newDate.getDate() + 1);
      this.currentDate = newDate;
      this.$emit('selectedDate', this.currentDate);
    },
    prevDay: function() {
      if (this.limitToCurrentWeek && this.currentDate.getDay() == 1) {
        return;
      }
      const newDate = new Date(this.currentDate);
      newDate.setDate(newDate.getDate() - 1);
